...
{
    public static class UtilitiesPgnUciConversions
    {
        public static string ConvertUciMoveToPgn(ChessBoard chessBoard, string uciMove)
        {
            string pgn = null;
            string uciFrom = uciMove.Substring(0, 2);
            string uciTo = uciMove.Substring(2, 2);
            string promotionChar = uciMove.Length > 4 ? uciMove.Substring(4,1) : null;
            ChessPiece pieceOnFromSquare = chessBoard.getChessPieceOnSquare(uciFrom);
            ChessPiece pieceOnToSquare = chessBoard.getChessPieceOnSquare(uciTo);

            ChessSquare chessSquareFrom = chessBoard.getChessSquareWithUciName(uciMove.Substring(0, 2));
            ChessSquare chessSquareTo = chessBoard.getChessSquareWithUciName(uciMove.Substring(2, 2));
            bool isEnpassantCapture = chessBoard.EnPassantTargetSquare != null &&
                                      chessSquareTo.Equals(chessBoard.EnPassantTargetSquare) &&
                                      chessSquareTo.PieceOnSquare.PieceType == PieceType.None &&
                                      chessSquareFrom.PieceOnSquare.PieceType == PieceType.Pawn &&
                                      chessSquareFrom.fileIndex != chessSquareTo.fileIndex;
            //bool isCapture =  (pieceOnToSquare != null) && (pieceOnToSquare.PieceType != PieceType.None);
            bool isCapture = isEnpassantCapture || (pieceOnToSquare.PieceType != PieceType.None);
            if ((pieceOnFromSquare != null) && (pieceOnFromSquare.PieceType != PieceType.None))
            {
                // Pawn move (must handle enpassant as well as all other pawn moves)
                if (pieceOnFromSquare.PieceType == PieceType.Pawn)
                {
                    pgn = isCapture ? pgn = uciFrom.Substring(0, 1) + "x" + uciTo : uciTo;
                    if (promotionChar != null)
                    {
                        pgn += "=" + promotionChar;
                    }
                }
                // King move (Castling move or normal king move)
                else if (pieceOnFromSquare.PieceType == PieceType.King)
                {
                    // handle castling moves
                    if ((pieceOnFromSquare.PieceColor == 'w') && (uciMove.Equals("e1g1"))) { pgn = "O-O"; }
                    if ((pieceOnFromSquare.PieceColor == 'w') && (uciMove.Equals("e1c1"))) { pgn = "O-O-O"; }
                    if ((pieceOnFromSquare.PieceColor == 'b') && (uciMove.Equals("e8g8"))) { pgn = "O-O"; }
                    if ((pieceOnFromSquare.PieceColor == 'b') && (uciMove.Equals("e8c8"))) { pgn = "O-O-O"; }
                    // handle normal king move
                    if (pgn == null)
                    {
                        pgn = isCapture ? "Kx" + uciTo : "K" + uciTo;
                    }
                }
                // Piece moves (non-pawn move and non-king move)
                else 
                {
                    char pieceTypeFilter = pieceOnFromSquare.FENCharForPiece.ToString().ToUpper().ElementAt(0);
                    ChessSquare fromChessSquare = chessBoard.getChessSquareWithUciName(uciFrom);
                    List<string> listLegalUciMoves = UtilitiesLegalUCIMoves.getAllLegalUciMovesForPieceToSquare(chessBoard, pieceTypeFilter, uciTo);
                    if (listLegalUciMoves.Count == 1)
                    {
                        pgn = isCapture ? pieceTypeFilter.ToString() + "x" + uciTo : pieceTypeFilter.ToString() + uciTo;
                    }
                    else if (listLegalUciMoves.Count == 2)
                    {
                        // must decide to use file or rank for disambiguation
                        // if both legal moves have same file then use rank for disambiguation
                        string fileOfLegalMove1 = listLegalUciMoves[0].Substring(0, 1);
                        string fileOfLegalMove2 = listLegalUciMoves[1].Substring(0, 1);
                        string disambiguationChar = fileOfLegalMove1.Equals(fileOfLegalMove2) ? uciFrom.Substring(1,1) : uciFrom.Substring(0,1);
                        pgn = isCapture ? 
                            pieceTypeFilter.ToString() + disambiguationChar + "x" + uciTo :
                            pieceTypeFilter.ToString() + disambiguationChar + uciTo;
                    }
                    else
                    {
                        pgn = isCapture ? pieceTypeFilter.ToString() + uciFrom + "x" + uciTo : pieceTypeFilter.ToString() + uciFrom + uciTo;
                    }
                }
            }
            return pgn;
        ...
        public static string ConvertPgnSANtoUCI(ChessBoard chessBoardBeforeMove, string pgnSAN)
        {
            ...
            if (pgnSAN != null)
            {
                ...
                // @ken fix 6/21/2023
                if (adjustedPgnSAN.EndsWith("+-") || adjustedPgnSAN.EndsWith("-+"))
                {
                    adjustedPgnSAN = adjustedPgnSAN.Substring(0, adjustedPgnSAN.Length - 2);
                }
        ...
        private static string ConvertPgnSANtoUCIForPawnPromotion(ChessBoard chessBoardBeforeMove, string adjustedPgnSAN)
        {
            ...
            promotionPortionPGN = adjustedPgnSAN.Substring(adjustedPgnSAN.IndexOf('=') + 1, 1).ToLower();

...
namespace KenChessPGNCoreLibrary
{
    public static class PGNTokenizer
    {
        ...
        public static List<PGNToken> tokenizeDataFromRawPGNMultigameText(string entireContent)
        {
            ...
            PGNToken gameTerminationToken = null;
            int ctrGames = 0;
            ...
            while (currentPosition < entireContent.Length)
            {
                ...
                // Must check for game termination token before checking for IntegerToken or Comments or any other token type
                if (TryGetGameTerminationToken(currentPosition, entireContent, out gameTerminationToken))
                {
                    ...
                    if (ctrGames > 200)
                    {
                        throw new Exception("Error! KenChessPGNUtilities does not allow more than 200 games in a single pgn file.");
                    }
                }
        ...
        /// <summary>
        /// A return value indicates if a game termination token was found.
        /// </summary>
        private static bool TryGetGameTerminationToken(int currentPosition, string entireContent, out PGNToken pgnToken)
        {
            pgnToken = null;

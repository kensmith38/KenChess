...
namespace KenChessBoardModel
{
    ...
    public class ChessBoard
    {
        ...
        public ChessBoard(string fenPosition) 
        {
            ...
            string uciSquareName;
            // initialize all squares with null pieces
            for (int fileIndex = 0; fileIndex < 8; fileIndex++)
            {
                for (int rankIndex = 0; rankIndex < 8; rankIndex++)
                {
                    uciSquareName = GetUciSquareName(fileIndex, rankIndex);
        ...
        public void PerformUciMove(string uciMove)
        {
            ...
            bool isEnpassantCapture = EnPassantTargetSquare != null &&
                                      chessSquareTo.Equals(EnPassantTargetSquare) && 
            ...
            bool isCapture = isEnpassantCapture || chessSquareTo.PieceOnSquare.PieceType != PieceType.None;
            EnPassantTargetSquare = null;
            ...
            if (isEnpassantCapture)
            {
                ...
                chessSquareFrom.PieceOnSquare = new ChessPiece();
                // Tricky! Remove the captured pawn which was captured but does not reside on the chessSquareTo!
                ChessSquare squareWithPawnToRemove = getAdjacentSquare(chessSquareTo, ActivePlayer == 'w' ? Direction.South : Direction.North);
                squareWithPawnToRemove.PieceOnSquare = new ChessPiece();
            }
            // All moves excluding castling and excluding pawn enpassant
            if (!isEnpassantCapture && !isCastling)
            {
                ...
                if (chessSquareTo.PieceOnSquare.PieceType == PieceType.Pawn)
                {
                    ...
                    {
                        // Tricky! Pawn promotion is weird in UCI... Lowercase q can mean either black or white queen!
                        chessSquareTo.PieceOnSquare.PieceColor = chessSquareTo.UciSquareName.EndsWith("1") ? 'b' : 'w';
                        switch (uciMove[4])
                        {
                            case 'q':
                                chessSquareTo.PieceOnSquare = new ChessPiece(PieceType.Queen, chessSquareTo.PieceOnSquare.PieceColor);
                                break;
                            case 'r':
                                chessSquareTo.PieceOnSquare = new ChessPiece(PieceType.Rook, chessSquareTo.PieceOnSquare.PieceColor);
                                break;
                            case 'b':
                                chessSquareTo.PieceOnSquare = new ChessPiece(PieceType.Bishop, chessSquareTo.PieceOnSquare.PieceColor);
                                break;
                            case 'n':
                                chessSquareTo.PieceOnSquare = new ChessPiece(PieceType.Knight, chessSquareTo.PieceOnSquare.PieceColor);
                                break;
                            default:
                                break;
                        }
        ...
        private void performCastling(string uciMove)
        {
            ...
            switch (uciMove)
            {
                case "e1c1":    // white long castle
                    getChessSquareWithUciName("a1").PieceOnSquare = new ChessPiece();
                    getChessSquareWithUciName("d1").PieceOnSquare = new ChessPiece('R');
                    getChessSquareWithUciName("e1").PieceOnSquare = new ChessPiece();
                    getChessSquareWithUciName("c1").PieceOnSquare = new ChessPiece('K');
                    break;
                case "e1g1":    // white short castle
                    getChessSquareWithUciName("h1").PieceOnSquare = new ChessPiece();
                ...
                case "e8c8":    // black long castle
                    getChessSquareWithUciName("a8").PieceOnSquare = new ChessPiece();
                    getChessSquareWithUciName("d8").PieceOnSquare = new ChessPiece('r');
                    getChessSquareWithUciName("e8").PieceOnSquare = new ChessPiece();
                    getChessSquareWithUciName("c8").PieceOnSquare = new ChessPiece('k');
                    break;
                case "e8g8":    // black short castle
                    getChessSquareWithUciName("h8").PieceOnSquare = new ChessPiece();
        ...
        /// </summary>
        public ChessSquare getChessSquareWithUciName(string uciChessSquareName)
        {
            ChessSquare chessSquare = null;
            char fileLetter = uciChessSquareName[0];
            char rankLetter = uciChessSquareName[1];
            int fileIndex = "abcdefgh".IndexOf(fileLetter);
            int rankIndex = "12345678".IndexOf(rankLetter);
        ...
        /// <summary>
        /// Gets the UCI square name for specific file index and rank index (ex: "a1" for fileIndex=0 rankIndex=0).
        /// FileIndex and RankIndex are zero based and assume the board is not flipped.
        /// </summary>
        public static string GetUciSquareName(int chessboardFileIndex, int chessboardRankIndex)
        {
            string squareName = null;
            squareName = "abcdefgh".Substring(chessboardFileIndex, 1) + (chessboardRankIndex + 1).ToString();
            return squareName;
        }
        /// <summary>
